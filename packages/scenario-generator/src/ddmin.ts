...
export async function ddmin<T>(input:T,features:readonly Feature<T>[],fails:(x:T)=>Promise<boolean>):Promise<{value:T;removed:readonly string[]}>{
 ...
 while(changed){changed=false;for(const f of features){const candidate=f.remove(value);if(await fails(candidate)){value=candidate;removed.push(f.id);changed=true}}}
