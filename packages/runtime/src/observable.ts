...
export class ObservableFlightRuntime{
 ...
 constructor(readonly pilot:CognitivePilot){}
 async run(scenario:Scenario,maxTicks=7200){
  ...
  for(let i=0;i<maxTicks;i++){
   if(i>=active.until){
    ...
    const frame:DecisionFrame={id:d.decisionId,startTick:this.sim.tick,requestedIntent:d.intent,executedIntent:safe.executed,provider:this.pilot.engines.primary.identity.provider,probability:d.probability};
