...
// Local deterministic PRNG (mulberry32): presentation code must not depend on @flight/simulation.
...
const next=mulberry32(0x5ce4e5),rand=(a=0,b=1)=>a+(b-a)*next();
...
export function buildScenery(root:THREE.Object3D,scene:THREE.Scene,renderer:THREE.WebGLRenderer):Scenery{
 ...
 const terrain=new THREE.Mesh(tg,new THREE.MeshStandardMaterial({vertexColors:true,roughness:.95,flatShading:false}));terrain.receiveShadow=true;root.add(terrain);
 ...
 runway.material.map!.repeat.set(1,1);runway.rotation.x=-Math.PI/2;runway.position.copy(toThree(RUNWAY.x,.08,mid));runway.receiveShadow=true;root.add(runway);
 const asphalt=new THREE.MeshStandardMaterial({color:0x4a4d52,roughness:.9});
 const taxi=new THREE.Mesh(new THREE.PlaneGeometry(14,len-120),asphalt);taxi.rotation.x=-Math.PI/2;taxi.position.copy(toThree(-70,.07,mid));taxi.receiveShadow=true;root.add(taxi);
 for(const z of [RUNWAY.zStart+80,RUNWAY.zEnd-80]){const t=new THREE.Mesh(new THREE.PlaneGeometry(70,12),asphalt);t.rotation.x=-Math.PI/2;t.position.copy(toThree(-35,.07,z));root.add(t)}
 const apron=new THREE.Mesh(new THREE.PlaneGeometry(90,180),asphalt);apron.rotation.x=-Math.PI/2;apron.position.copy(toThree(-125,.06,120));apron.receiveShadow=true;root.add(apron);
 const shadowed=(m:THREE.Mesh)=>{m.castShadow=true;m.receiveShadow=true;root.add(m);return m};
 ...
 conifer.count=nc;broad.count=nb;trunks.count=nt;for(const m of [conifer,broad]){m.castShadow=true;m.receiveShadow=true;root.add(m)}root.add(trunks);
 ...
 barn.count=nbarn;barn.castShadow=true;root.add(barn);
