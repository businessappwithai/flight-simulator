...
const bad:string[]=[];
for(const pkg of protectedPkgs)for(const f of (await walk(`packages/${pkg}`)).filter(x=>x.endsWith(".ts"))){const t=await Bun.file(f).text();if(forbidden.some(x=>t.includes(x)))bad.push(f)}
