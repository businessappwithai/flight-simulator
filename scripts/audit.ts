...
async function walk(d:string):Promise<string[]>{const out:string[]=[];for(const x of await readdir(d,{withFileTypes:true})){const p=join(d,x.name);x.isDirectory()?out.push(...await walk(p)):out.push(p)}return out}
